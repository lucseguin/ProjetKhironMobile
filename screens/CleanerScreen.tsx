...
export default function CleanerScreen() {
  ...
  const [testServiceLevel, setTestServiceLevel] = useState(16);
  const [dataForServiceLevel, setDataForServiceLevel] = useState(16);
  ...
  useEffect(() => {
    //console.log("CleanerScreen Mounted");
    
    EventCoordinator.register('notif', ({ payload }) => {
      //console.log("[CleanerScreen] notif signal");
      if(payload.data.type  === "cleaner") {
    ...
    return () => {
      clearInterval(ti);
      //console.log("CleanerScreen Unmounted");
    }
  }, []);
  ...
  function handleAcceptRequest(request:any) {
    ...
    axios.put("/projetkhiron/cleaner/request/accept", {
  ...
  function handleCompletedRequest(request:any) {
    setIsPerformingAction(true);
    axios.put("/projetkhiron/cleaner/request/completed", {
      requestId:request._id,
  ...
  const initialLoadOOSStats = () => {
    axios.get("/projetkhiron/roles", {
      ...
      console.log("Failed to reteive Cleaner role settings for OOS Stats. " + error.message);
    }).finally(() => {
  ...
  const loadOOSRequests = (sLevel:number, trackLoading:boolean) => {
    if(trackLoading)
      setIsPerformingAction(true);
    axios.get("/projetkhiron/cleaner/analysis", {
      params: {
        type:'outOfService',
        seviceLevel: sLevel
        }
...
var oosStatsView = null;
  if(globalAny.settings.options&ModuleSettings.ROLE_MOBILE_OOS_STATS) {
    oosStatsView = <Collapse key="cleaner-mobile-oos-stats" style={{borderBottomWidth:1, borderColor:theme.SeparatorColor}}>
  ...
  if((globalAny.settings.options&ModuleSettings.ROLE_MOBILE_HIDE_REQUESTS&&UserData.isShowCurrentRequests()) || !(globalAny.settings.options&ModuleSettings.ROLE_MOBILE_HIDE_REQUESTS)) {
    if(allRequests.length>0){
        ...
        <Collapse key={request._id} style={{borderBottomWidth:1, borderColor:theme.SeparatorColor}}>
          <CollapseHeader style={{ flexDirection:'row',alignItems:'flex-start',padding:10,backgroundColor:theme.SecondarySystemBackgroundColor}}>
            ...
            <View style={{width:width-60, alignItems:'flex-start', backgroundColor:theme.SecondarySystemBackgroundColor}}>
              <Text>Pour:   {request.from.label} </Text>
          ...
          <CollapseBody style={{backgroundColor:theme.TertiarySystemBackgroundColor}}>
            ...
            : globalAny.settings.options&ModuleSettings.MODULE_CLEANER_UPDATE?<Button icon={<Icon name="directions-run" size={20} color="white" />} title="Accepter" onPress={() => handleAcceptRequest(request)}/>:null 
            }
            </ListItem>
            ...
            :(request.assignedOn!=='1970-01-01T00:00:00.000Z')?(globalAny.settings.options&ModuleSettings.MODULE_CLEANER_UPDATE?<Button icon={<Icon name="done" size={20} color="white" /> } title="Terminer" onPress={() => handleCompletedRequest(request)}/>:null):null}
            </ListItem>
          </CollapseBody>
        </Collapse>
        ));
      ...
      currentRequestsView = <Collapse style={{borderBottomWidth:1, borderColor:theme.SeparatorColor}}>
        <CollapseHeader style={{ flexDirection:'row',alignItems:'flex-start',padding:10,backgroundColor:theme.SecondarySystemBackgroundColor}}>
          <View style={{width:width-20, alignItems:'flex-start', backgroundColor:theme.SecondarySystemBackgroundColor}}>
            <Text>Aucune demande de nettoyage dans les derniers 8 heures</Text>
          </View>
        </CollapseHeader>
      </Collapse>;
    }
